...
export default function SelectPanel({
  ...
  useEffect(() => {
    if (Array.isArray(initialChoices) && initialChoices.length > 0) {
      ...
      const seeded = initialChoices.map((c, idx) => ({
        id: c.id || Date.now() + idx,
        class_name: c.class_name || "",
        count_condition: c.count_condition || ">0",
        active: !!c.active,
      }));
      setChoices(seeded);
    }
  ...
  useEffect(() => {
    const active = choices.find((c) => c.active) || {
      class_name: "",
      count_condition: ">0",
      active: false,
    };
  ...
  const handleChangeClass = (idx, name) => {
    ...
    const isDuplicate = choices.some(
      (c, i) =>
        i !== idx &&
        (c.class_name || "").trim().toLowerCase() === nextName.toLowerCase()
    );
    if (isDuplicate) {
  ...
  const handleTogglePos = (idx) => {
    setChoices((prev) =>
      prev.map((c, i) => ({ ...c, active: i === idx ? !c.active : false }))
    );
  };
  ...
  const handleAddChoice = () => {
    setChoices((prev) => [
      ...prev,
      { class_name: "", count_condition: ">0", active: false, id: Date.now() },
    ]);
  };
      ...
      {choices.map((c, idx) => (
        <div key={c.id} className="odsp-class-card">
          <div className="odsp-class-row">
            ...
            <button
              type="button"
              className="btn-secondary odsp-remove-btn"
