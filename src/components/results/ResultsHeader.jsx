  ...
  const handleApplyFilters = () => {
    if (isFiltersApplied) {
      ...
      if (includeList.length > 0) {
        ...
        // Remove duplicates
        filteredResults = includedResults.filter((result, index, self) => 
          index === self.findIndex(r => r.video_name === result.video_name && r.frame_idx === result.frame_idx)
        );
      }
      
      console.log(`Filtered from ${originalSearchResults.length} to ${filteredResults.length} results`);
      ...
      <div className="results-header-top">
        ...
        <div className="video-search-controls">
          ...
          <div className="apply-filters-section">
            <button 
              className={`apply-btn ${isFiltersApplied ? 'applied' : 'not-applied'}`} 
